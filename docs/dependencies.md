...
| `integration_test` | SDK (dev) | 3.47.5 | BSD-3-Clause | flutter.dev | The binding behind `integration_test/`: drives the installed app on a simulator and hands frame timings and a report back (`IntegrationTestWidgetsFlutterBinding`). Part of the Flutter SDK, so it carries no version of its own. It pulls the driver side into the lock file: `webdriver` 3.2.0 (Apache-2.0, google.com), `sync_http` 0.3.1 (BSD-3-Clause, google.com) and `process` 5.0.6 (BSD-3-Clause, tools.dart.dev) — dev-only, never in the app; `webdriver` and `sync_http` are the web/driver half of the package and are not reached from a simulator run. Added by WP-41. |
