...
| --- | --- | --- | --- | --- | --- |
| `gql_http_link` | 1.2.0 (2025-09-20) | MIT | gql-dart.dev | Already in the lock file through `graphql`. Would become a direct dependency should `graphql` have to go: the links, `ApiExecutor` and the generated code only need `gql_link`, `gql_exec` and an HTTP link. | – |
